...
Since this bound holds for any $t\in\mathbb{R},$ we have shown that each element of $\nabla\mathcal{L}_{n}(\theta^{\ast})$ has zero-mean and sub-Gaussian with parameter at most $BC/\sqrt{n}.$ Thus, sub-Gaussian tail bounds combined with the union bound guarantee that
