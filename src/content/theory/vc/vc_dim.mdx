...
$$
\mathbb{E}\left[\sup_{A\in\mathcal{A}}|\mu_{n}(A)-\mu(A)|\right]\leq2\frac{\sqrt{2\operatorname{VC}(\mathcal{A})\log(n+1)+\log(2)}}{n}.
$$
</details>
...
$$
\sup_{A\in\mathcal{A}}|\mu_{n}(\mathcal{A})-\mu(\mathcal{A})|	\leq2\frac{\sqrt{2\operatorname{VC}(\mathcal{A})\log(n+1)+\log(2)}}{n}+\sqrt{\frac{\log(1/\delta)}{2n}}.
$$
</details>
