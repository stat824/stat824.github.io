# Minimal sufficiency, sub-family lemma

We say that a statistic $S$ is minimal sufficient if it is sufficient and for every sufficient $T$, there is a measurable function $f$ such that $S=f(T)$. How do we find minimal sufficient statistic? One way is the sub-family method, which is based on the following observation. 
